...
- DICOMweb local con QIDO-RS, WADO-RS, STOW-RS básico y visor web liviano.
...
| `dicomweb_server.py` | API DICOMweb y visor web local |
...
## Visor web y DICOMweb

La pestaña **MiniPACS** también permite iniciar el servicio HTTP DICOMweb, por defecto en `http://127.0.0.1:8080`. El botón **Abrir visor** lanza un visor web liviano incluido en el proyecto. El visor muestra la lista de estudios y una previsualización de la primera instancia CT.

Endpoints disponibles:

| Endpoint | Función |
|---|---|
| `GET /dicom-web/studies` | QIDO-RS básico de estudios/instancias |
| `GET /dicom-web/studies/{study}/series` | Series de un estudio |
| `GET /dicom-web/studies/{study}/series/{series}/instances` | Instancias de una serie |
| `GET /dicom-web/studies/{study}/series/{series}/instances/{sop}` | WADO-RS multipart DICOM |
| `POST /dicom-web/studies` | STOW-RS básico para un DICOM `application/dicom` |

El servicio incluye CORS para permitir que un cliente externo se conecte durante el desarrollo. OHIF puede utilizar estos endpoints como base de una configuración externa de DICOMweb, aunque este proyecto incluye por defecto el visor liviano para evitar descargar y mantener una distribución completa de OHIF.
...
- DICOMweb usa Flask y WADO-RS devuelve las instancias almacenadas con `multipart/related; type=application/dicom`.
